...
export function StartScreen({ onLaunch }: StartScreenProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 left-6 w-16 h-px bg-gradient-to-r from-primary/40 to-transparent animate-pulse-glow" />
      <div className="absolute top-6 left-6 w-px h-16 bg-gradient-to-b from-primary/40 to-transparent animate-pulse-glow" />
      <div className="absolute top-6 right-6 w-16 h-px bg-gradient-to-l from-primary/40 to-transparent animate-pulse-glow" />
      <div className="absolute bottom-6 left-6 w-16 h-px bg-gradient-to-r from-primary/40 to-transparent animate-pulse-glow" />
      ...
      {/* Main content */}
      <div className="relative z-10 flex flex-col items-center gap-6 w-full max-w-6xl px-6">
        {/* Header */}
        <div
        ...
        >
          ...
          <h1 className="font-mono text-4xl md:text-5xl font-bold text-foreground text-glow-strong tracking-[0.2em] text-balance text-center">
            MISSION CONTROL
          </h1>
            ...
            <div className="glass-panel rounded-lg p-4">
              ...
              <div className="relative w-full aspect-square max-w-[160px] mx-auto">
                ...
                <svg viewBox="0 0 200 200" className="w-full h-full">
                  ...
                  <circle cx="120" cy="150" r="1" fill="rgba(255,204,0,0.6)" className="animate-pulse" />
                </svg>
                <div className="absolute bottom-0 left-0 right-0 text-center">
                  <span className="font-mono text-[8px] text-muted-foreground/50">5 OBJECTS DETECTED</span>
                </div>
              </div>
            </div>
            {/* Orbit status */}
            <div className="glass-panel rounded-lg p-3">
              ...
              <div className="space-y-1 font-mono text-[10px] text-muted-foreground/70">
                <div className="flex items-center justify-between">
                  <span>Apoapsis</span>
                  <span className="text-foreground/70">420.3 km</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Periapsis</span>
                  <span className="text-foreground/70">380.1 km</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Inclination</span>
                  <span className="text-foreground/70">51.6 deg</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Period</span>
                  <span className="text-foreground/70">92.4 min</span>
                </div>
              </div>
            </div>
          </div>

          {/* Center-Right: Gauges & Launch */}
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="glass-panel rounded-lg p-3">
              ...
              <div className="flex items-center justify-between mt-1">
                <span className="font-mono text-[8px] text-muted-foreground/40">20Hz</span>
                <span className="font-mono text-[8px] text-muted-foreground/40">20kHz</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-center gap-8 px-8 py-4">
        ...
        <TelemetryItem label="SIGNAL" value="98.2" unit="dB" />
      </div>
    </div>
  )
}
...
function TelemetryItem({ label, value, unit }: { label: string; value: string; unit: string }) {
  return (
    <div className="flex items-baseline gap-1.5">
      ...
      <span className="font-mono text-xs text-muted-foreground tracking-wider">{value}</span>
      {unit && <span className="font-mono text-[9px] text-muted-foreground/40">{unit}</span>}
    </div>
  )
}
